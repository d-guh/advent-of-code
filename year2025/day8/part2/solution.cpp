...
int main() {
    ...
    pair<int, int> final_union = {-1, -1};
    for (const auto& edge : all_pairs) {
        if (uf.unite(edge.u, edge.v)) {
            final_union = {edge.u, edge.v};
        }
    }
    //cout << "DEBUG: final_union: " << final_union.first << ", " << final_union.second << ": " << junction_boxes[final_union.first].x << " * " << junction_boxes[final_union.second].x << endl;
    unsigned long long product = junction_boxes[final_union.first].x * junction_boxes[final_union.second].x;
