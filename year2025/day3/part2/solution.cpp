...
int main() {
    ...
    while (getline(banks_file, bank)) {
        // These are chars for display purposes, but behave the same with the exception of the ascii conversion when comptuing combined_joltage.
        vector<int> batteries;

        for (size_t i = bank.size() - num_batteries; i < bank.size(); i++) {
            batteries.push_back(bank[i] - '0');  // Convert to int
        }
